...
class MultiModal:
    """
    텍스트와 이미지 메시지를 모델에 전달하고 처리하기 위한 다중 모달 유틸리티 클래스입니다.
    이 클래스는 URL 또는 파일 경로에서 이미지를 base64로 인코딩하고, Jupyter에서 이미지를 표시하며,
    텍스트와 이미지 프롬프트를 함께 포함하는 메시지를 구조화하는 기능을 제공합니다. 또한 단일 메시지로
    모델을 호출하거나, 여러 이미지 메시지를 일괄 처리하며, 스트림으로 모델 응답을 받을 수 있습니다.
